...
void setup() {
  ...
  if (network) {
    client = new Client(this, IP, 1245);
  } else {
...
void sendData() {
  ...
  //sends the data
  client.write(width - playerTank.currentPosition().x + " " + (width - mouseX) + " " + mouseY + " " + sendData + " " + missileVel + " " + playerTank.missileStrength + '\n' + 
               playerTank.currentPosition().y + " " + orientation + "PI");
  if (sendData.equals("1") || sendData.equals("2") ) {
...
void draw() {
  if (gameState.equals("title") ) {
    ...
    if (startTime == -1 && network == true && client.available() > 0 ) {
    ...
    if (network == true && client.available() > 0) {
      if (dataError == false) {
